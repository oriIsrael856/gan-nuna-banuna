...
## מה בוצע (2026-09-22) — פידבק ראשון מאיציק

1. **תיקון אייקונים חסרים:** איורים לכפתורי "העלאת חוזה" ו"דיווחי היעדרות", וטעינה מוקדמת של פונט האייקונים (ספארי באייפון).
2. **שעון נוכחות צוות:** מיגרציה `0014_staff_time_entries.sql`; מסך `/teacher/time-clock` (כניסה/יציאה + שעות החודש); למנהל `/admin/staff-hours` (כל העובדים) ו-`/admin/staff-hours/[id]` (פירוט עובד). בתפריט התחתון של הצוות "תיעוד" הוחלף ב"כניסה/יציאה".
3. **דיווח אישי פר-ילד:** מיגרציה `0015_child_daily_updates.sql` (RLS: הורה רואה רק את הילד שלו); גננת: `/teacher/child-updates` (רשימה) → `/teacher/child-update?childId=` (ארוחות, שינה, מצב רוח, החתלות, הערה); הורה: כרטיס "היום של {ילד}" בראש הסיכום היומי.
4. תמיכת "הוסף למסך הבית" באייפון (`+html.tsx`, `public/apple-touch-icon.png`).

⚠️ המיגרציות 0014 ו-0015 חייבות לרוץ ב-SQL Editor לפני פריסת הגרסה.
